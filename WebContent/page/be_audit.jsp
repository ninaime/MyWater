	...
	<table class="data display">
	  ...
	  <tbody>
		...
		<c:forEach items="${list}" var="list">
		  <tr class="odd">
			<td><a href="/MyWater/BeApply/OrderInfo?id=${list.orderno}" target="orderInfo">${list.orderno}</a></td>
			...
			<td><button class="btn-icon btn-small btn-blue btn-check" 
			  onClick="location.href = '/MyWater/BeApply/AuditForm?id=${list.orderno}';">
			  <span></span>处理</button>
			</td>
			</tr>
		  </c:forEach>
	  ...
	  <div class="page">
		<c:choose>
   		  <c:when test="${page.pages==1}">
   			首页
   			上一页
   			<a href="../BeApply/Auditing?pages=${page.pages+1}">下一页</a>
   			<a href="../BeApply/Auditing?pages=${page.last}">末页</a>
   		  </c:when>
   		  <c:when test="${page.pages==page.last}">
   			<a href="../BeApply/Auditing?pages=1">首页</a>
   			<a href="../BeApply/Auditing?pages=${page.pages-1}">上一页</a>
   			下一页
   			末页
   		  </c:when>
   		  <c:otherwise>
   		...
   		<form action="${pageContext.request.contextPath}/BeApply/Auditing" method="post">
