...
<!-- #header -->	
  ...
  <div id="content" class="xgrid">
	<form action="/Water/sy_metertype/update?id=${sm.id}" method="post">
 	  <div class="x12">
	  <h2>添加/修改水表型号</h2>
		<div class="form label-inline uniform">
	   	  <div class="field"><label for="id">水表型号ID</label> 
	   	    <input id="id" name="fname" size="50" type="text" 
	   	    class="medium" disabled="disabled" value="${sm.id}" />
	   	  </div>
		  <div class="field"><label for="name">水表型号</label> 
		    <input id="name" name="meterTypeName" size="50" type="text" 
		    class="medium" value="${sm.metertypename}" onblur="show()"/>
		  </div>
		  <div class="field"><label for="name">设置口径</label> 
		    <input id="aperture" name="aperture" size="50" type="text" 
		    class="medium" value="${sm.aperture}" onblur="show2()"/>
		  </div>
		  <div class="field"><label for="name">最大表码</label> 
		    <input id="mavValue" name="mavValue" size="50" type="text" 
		    class="medium" value="${sm.mavvalue}" onblur="show3()"/>
		  </div>
		  <div class="field"><label for="name">下限设置</label> 
		    <input id="minValue" name="minValue" size="50" type="text" 
		    class="medium" value="${sm.minvalue}"/> 吨
		  </div>
		  <div class="field"><label for="name">周检年限</label> 
		    <input id="life" name="life" size="50" type="text" 
		    class="medium" value="${sm.life}" onblur="show4()"/>月
		  </div>
		  <div class="field"><label for="description">备注</label> 
		    <textarea rows="7" cols="50" id="description" name="remark" 
		    onblur="show5()">${sm.remark}</textarea>
		  </div>
		  <div class="buttonrow">
