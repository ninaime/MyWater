	  ...
	  <tbody>
		...
		<c:forEach items="${list}" var="list">
		  <tr class="odd">
			...
			<td>${list.time}</td>
			<td>${list.username}</td>
			<td><button class="btn-icon btn-small btn-blue btn-check" onClick="location.href = '/Water/order/billclearForm?id=${list.orderno}';"><span></span>处理</button></td>
		  </tr>
		</c:forEach>
	  </tbody>
	</table>
	<div class="page">
	  <c:choose>
   		<c:when test="${page.pages==1}">
   		        首页
   		        上一页
   		  <a href="../BeApply/Billclear?pages=${page.pages+1}">下一页</a>
   		  <a href="../BeApply/Billclear?pages=${page.last}">末页</a>
   		</c:when>
   		<c:when test="${page.pages==page.last}">
   		  <a href="../BeApply/Billclear?pages=1">首页</a>
   		  <a href="../BeApply/Billclear?pages=${page.pages-1}">上一页</a>
   			下一页
   			末页
   		</c:when>
   		<c:otherwise>
   	  ...
   	  <form action="${pageContext.request.contextPath}/BeApply/Billclear" method="post">
	...
	if (sum != 0) {
		if (sum > ye) {
			...
			if (sum > 0 && sum <= ye) {
				...
				//var path = "role.do?method=roleFY&shuju=" + shuju + "&ye="
				//		+ sum;
				var path="/Water/order/billclear?id="+sum;
